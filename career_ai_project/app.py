...
Logic mô hình đã được chuyển sang file model.py để dễ quản lý.
...
def render_profile_dashboard(features: dict) -> None:
    ...
    with col1:
        st.metric("Ngành học", features["Field"])
    with col2:
...
if analyze_clicked:
    ...
    try:
        vector = build_feature_vector(features, field_encoder, warning_callback=st.warning)
